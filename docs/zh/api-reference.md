...
本文档面向自研 Gateway、服务网格适配器、内部平台控制面和后端服务接入。普通业务接入优先看 [接入指南](integration-guide.md) 和各语言中间件 README。
...
| `GET` / `POST` | `/api/v1/admin/applications` | 应用列表和保存；新应用响应仅一次返回 `client_secret`。 |
