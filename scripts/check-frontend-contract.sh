...
require_pattern web/admin/vite.config.ts '"Content-Security-Policy": policies\.header' "admin dev and preview servers emit CSP headers"
require_pattern web/admin/vite.config.ts '"X-Frame-Options": "DENY"' "admin dev and preview servers deny framing"
require_pattern deploy/nginx/captcha-admin.conf.example "frame-ancestors 'none'" "admin nginx example denies framing with a response header"
